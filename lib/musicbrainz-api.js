    ...
    */
    async mergeEntities(entity, targetid, mbids) {
        if (!mbids.includes(targetid))
            mbids.push(targetid);
        const mergeFormData = { 'merge.edit_note': 'same work' };
        await this.login();
        await this.rateLimiter.limit();
        const url = `${entity}/merge`;
        const r = await got_1.default.post(url, Object.assign({ body: queryString.stringify({ submit: 'cancel' }), followRedirect: false }, this.options));
        await this.rateLimiter.limit();
