...
class MusicBrainzApi {
    ...
    * @param mbid
    */
    async mergeEntities(entity, targetid, mbids, editNote) {
        if (!mbids.includes(targetid))
            mbids.push(targetid);
        const mergeFormData = { 'merge.edit_note': editNote };
        await this.login();
        await this.rateLimiter.limit();
        const url = `${entity}/merge`;
        const r = await got_1.default.post(url, Object.assign({ form: { submit: 'cancel' }, followRedirect: false }, this.options));
        await this.rateLimiter.limit();
        ...
        // merging
        const response = await got_1.default.post(url, Object.assign({ form: mergeFormData, followRedirect: false }, this.options));
        if (response.statusCode === http_status_codes_1.StatusCodes.OK)
            throw new Error(`Failed to submit form data`);
        if (response.statusCode === http_status_codes_1.StatusCodes.MOVED_TEMPORARILY) {
            if (!response.headers || !response.headers.location)
